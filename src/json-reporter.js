...
const {
  ...
  Test,
  TestCode,
} = require('@saucelabs/sauce-json-reporter');
  ...
  return {
    ...
    reportTestDone(testName, testRunInfo, meta) {
      testRunInfo.browsers.forEach((browser) => {
        ...
        const errs = testRunInfo.errs
          .filter(idFilter)
          .map((err) => this.formatError(err));
        const codes = testRunInfo.errs
          .filter(idFilter)
          .map((err) => this.formatErrorCallsite(err));
        const screenshotAssets = testRunInfo.screenshots
          .filter(idFilter)
          .map((s) => this._getAsset(s.screenshotPath, testName));
        ...
        if (this.videoStartTime) {
          test.videoTimestamp =
            (testStartTime.getTime() - this.videoStartTime) / 1000;
        }
        ...
        this.currentFixture.addTestWithAssets(
          browser.prettyUserAgent,
          test,
          screenshotAssets,
          videoAssets,
        );
